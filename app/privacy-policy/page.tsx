export default function PrivacyPolicyPage() {
  return (
    <main style={{ maxWidth: 720, margin: "48px auto", padding: "0 20px", fontFamily: "system-ui, sans-serif", lineHeight: 1.6 }}>
      <h1>Privacy Policy</h1>
      <p><strong>Effective Date:</strong> February 1, 2026</p>
      <p><strong>Company:</strong> EYWA Consulting Services Inc (d/b/a Estate AI)</p>

      <h2>1. Overview</h2>
      <p>
        Estate AI is a real estate communication platform operated by EYWA Consulting Services Inc.
        We provide SMS and WhatsApp messaging services on behalf of licensed real estate agents
        at Keller Williams Commercial to connect with property owners regarding potential real
        estate opportunities. This Privacy Policy describes how we collect, use, and protect
        your personal information.
      </p>

      <h2>2. Information We Collect</h2>
      <ul>
        <li><strong>Contact Information:</strong> Phone numbers, email addresses, and names provided by property owners or obtained from public property records.</li>
        <li><strong>Message Content:</strong> Text messages sent to and received from our messaging services.</li>
        <li><strong>Property Information:</strong> Publicly available property addresses and ownership details from county records.</li>
        <li><strong>Usage Data:</strong> Message delivery status, timestamps, and interaction history for service quality purposes.</li>
      </ul>

      <h2>3. How We Use Your Information</h2>
      <ul>
        <li>To send real estate-related communications about potential property opportunities.</li>
        <li>To respond to your inquiries and messages.</li>
        <li>To schedule meetings and follow-ups at your request.</li>
        <li>To comply with legal and regulatory requirements.</li>
        <li>To improve our messaging services and communication quality.</li>
      </ul>

      <h2>4. Consent and Opt-In</h2>
      <p>
        We only send messages to individuals who have provided their consent to receive
        communications from our real estate agents. Consent is obtained through one or more
        of the following methods:
      </p>
      <ul>
        <li>Submitting a property inquiry form on our website or a partner website.</li>
        <li>Providing your phone number directly to a Keller Williams Commercial agent.</li>
        <li>Responding to a direct mail piece that includes SMS opt-in language.</li>
        <li>Verbally consenting during a phone call or in-person meeting with an agent.</li>
      </ul>

      <h2>5. Message Frequency and Charges</h2>
      <p>
        Message frequency varies based on your interaction with our agents. Typically,
        you may receive 1-4 messages per month. Message and data rates may apply depending
        on your mobile carrier and plan.
      </p>

      <h2>6. Opt-Out</h2>
      <p>
        You may opt out of receiving messages at any time by replying <strong>STOP</strong> to
        any message. After opting out, you will receive a single confirmation message and no
        further messages will be sent. You may also opt out by contacting us directly at the
        information below. To resume messages, reply <strong>START</strong>.
      </p>

      <h2>7. Data Sharing</h2>
      <p>
        We do not sell, rent, or share your personal information with third parties for
        marketing purposes. Your information may be shared only with:
      </p>
      <ul>
        <li>The Keller Williams Commercial agent assigned to your inquiry.</li>
        <li>Service providers necessary to deliver messages (e.g., Twilio, Meta/WhatsApp).</li>
        <li>Law enforcement or regulatory bodies when required by law.</li>
      </ul>

      <h2>8. Data Security</h2>
      <p>
        We implement industry-standard security measures including encryption, access controls,
        and secure data storage to protect your personal information from unauthorized access,
        alteration, or disclosure.
      </p>

      <h2>9. Data Retention</h2>
      <p>
        We retain your information for as long as necessary to provide our services and comply
        with legal obligations. You may request deletion of your data at any time by contacting
        us at the information below.
      </p>

      <h2>10. Contact Us</h2>
      <p>
        For questions or concerns about this Privacy Policy or your personal data, contact us at:
      </p>
      <ul>
        <li><strong>Company:</strong> EYWA Consulting Services Inc</li>
        <li><strong>Email:</strong> privacy@eywaconsulting.com</li>
        <li><strong>Phone:</strong> (848) 456-9428</li>
      </ul>

      <p style={{ marginTop: 40, fontSize: 14, color: "#666" }}>
        This policy may be updated from time to time. Changes will be posted on this page
        with an updated effective date.
      </p>
    </main>
  );
}
